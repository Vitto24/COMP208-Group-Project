...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            grant_admin = form.cleaned_data.get('grant_admin_access', False)

            # Map role onto Django's is_staff/is_superuser flags
            if role == 'admin':
                user.is_staff = True
            ...
            # Lecturers/admins skip the module picker — nothing to finish
            needs_modules = role == 'student'

            UserProfile.objects.create(
                user=user,
                role=role,
                course=form.cleaned_data.get('course'),
                year_of_study=form.cleaned_data.get('year_of_study') or 1,
                university=form.cleaned_data.get('university', 'uol'),
                registration_complete=not needs_modules,
            )
